...
專案根目錄為本 repo 的 **`chord_ui/`**（內含 `backend/`、`frontend/`、根目錄的 `experiment.py` 等）。
...
- **IGTGS**：建議與 `chord_ui` **同層目錄**放置 `IGTGS/` 或 `igtgs/`（內含 `igtgs_backend/`）；程式會自動解析，或以環境變數指定
...
在 **`chord_ui` 根目錄**建立 `.env`（與 `experiment.py` 同層）。變數會由 `backend/app/settings.py` 載入（不覆寫已存在之環境變數）。
...
| `AUDIO_SESSIONS_DIR` | （選用）自訂音訊快取目錄；未設則使用 `.chord_ui_cache/audio_sessions/`。 |

詳見 `backend/app/settings_example.py`。請勿將含私密路徑的 `.env` 提交至版本庫。

## 啟動後端

在 **`chord_ui` 根目錄**執行（以便讀取 `.env` 與呼叫根目錄 `experiment.py`）：
