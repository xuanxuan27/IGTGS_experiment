...
# Removed train_eval_test_split import - not needed for inference
...
class ReweightedLoss(nn.Module):
    ...
    def __init__(self,counter,power=1.0,max_clip=10.0,gpu=False,triad_only=False,device=None):
        super(ReweightedLoss, self).__init__()
        self.weight=[None]*6
        # ENHANCED GPU ACCELERATION: Support device-aware tensor operations
        self.device = device if device is not None else (torch.device('cuda') if gpu else torch.device('cpu'))
        ...
        for i in range(6):
            ...
            if(gpu==True or device is not None):
                # FIXED: Use device-aware tensor operations instead of hardcoded .cuda()
                self.weight[i]=self.weight[i].to(self.device)
...
class ChordNet(NetworkBehavior):
    ...
    def __init__(self,cross_subpart_counter,triad_only=False):
        ...
        if(cross_subpart_counter is not None):
            # ENHANCED GPU ACCELERATION: Pass device information to ReweightedLoss
            self.loss_reweight=ReweightedLoss(cross_subpart_counter,power=1.0,max_clip=1.0,gpu=self.use_gpu,triad_only=triad_only,device=self.device)
...
class ChordNetCNN(NetworkBehavior):
    ...
    def __init__(self,cross_subpart_counter):
        ...
        if(cross_subpart_counter is not None):
            # ENHANCED GPU ACCELERATION: Pass device information to ReweightedLoss
            self.loss_reweight=ReweightedLoss(cross_subpart_counter,power=1.0,max_clip=1.0,gpu=self.use_gpu,device=self.device)
    def init_hidden(self,batch_size,hidden_dim):
        c_0=torch.zeros(2,batch_size,hidden_dim//2)
        h_0=torch.zeros(2,batch_size,hidden_dim//2)
        if(self.use_gpu):
...
if __name__ == '__main__':
    ...
    if(0<=slice_id and slice_id<=5):
        ...
        f.close()
        # Note: train_eval_test_split functions removed for production inference
        # For training, you would need to restore the original train_eval_test_split.py
        train_indices=np.arange(song_count)  # Fallback for inference-only deployment
        val_indices=np.arange(0,1)  # Fallback for inference-only deployment
    else:
